...
//}
double GetOverlap(const Atom& atom1, const ContractedGaussianOrbital& orbital1, const Atom& atom2, const ContractedGaussianOrbital& orbital2)
{
	double res = 0;
	...
	for (auto& gaussian1 : orbital1.gaussianOrbitals) 
		for (auto& gaussian2 : orbital2.gaussianOrbitals)
		{
			QuantumNumbers maxQN1 = atom1.GetMaxQN(gaussian1.alpha);
			QuantumNumbers maxQN2 = atom2.GetMaxQN(gaussian2.alpha);
			GaussianMoment moment(gaussian1.alpha, gaussian2.alpha, atom1.position, atom2.position, maxQN1, maxQN2);
...
Eigen::MatrixXd OverlapMatrix(std::span<Atom> atoms)
{
	...
	for (const Atom& atom : atoms)
		numberOfContractedGaussians += atom.NumberOfContractedGaussians();
				...
				for (const auto& atom2 : atoms)
				{
					for (const auto& shell2 : atom2.shells)
					{
						for (const auto& orbital2 : shell2.basisFunctions)
						{
							if (j >= i)
							{
								overlapMatrix(i, j) = GetOverlap(atom1, orbital1, atom2, orbital2);
								if (i != j) 
